...
class Header(
    namedtuple(
        'Header',
        'name content order orderable weight adjust tooltip'
    )
):
    __slots__ = ()
    ...
    def __str__(self):
        return mark_safe(
            '<th width="{w:.5f}" {a} {o}>{n}</th>'.format(
                ...
                t='' if not self.tooltip else 'data-toggle="{}"'.format(
                    self.tooltip)
            ))
    ...
    def fit(self, data):
        return mark_safe(
            '<td {a} {o}>{c}</td>'.format(
                a='' if not self.adjust else 'align="{}"'.format(self.adjust),
                o='' if not self.order else 'data-order="{}"'.format(
                    self.order(data)),
                c=self.content(data)
            ))
...
class Headers:
    ...
    def add_column(self, name, content, order=None, orderable=True,
        ...
        self.headers.append(Header(name, content, order,
                                   orderable, weight, adjust, tooltip))
